...
int memory_register_native_methods(JNIEnv *env) {
  jclass cls;
  if ((cls = (*env)->FindClass(env, "memories/jdk2/JDK2Memory$NativeMemoryAccess")) == NULL) {
    fprintf(stderr, "FATAL: Class memories.jdk2.JDK2Memory$NativeMemoryAccess not found");
    fflush(stderr);
    return JNI_ERR;
  }
