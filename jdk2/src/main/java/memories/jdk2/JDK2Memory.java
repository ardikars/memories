...
class JDK2Memory implements Memory {
  ...
  static {
    BE = JDK2MemoryAllocator.NativeMemoryAllocator.nativeByteOrderIsBE();
  }
  ...
  public Memory getBytes(long index, Memory dst, long dstIndex, long length) {
    ...
    long dstAddr = ((JDK2Memory) dst).address;
  ...
  public Memory setBytes(long index, Memory src, long srcIndex, long length) {
    ...
    long srcAddr = ((JDK2Memory) src).address;
  ...
  public Memory copy(long index, long length) {
    ...
    JDK2Memory newBuf = (JDK2Memory) allocator.allocate(length, byteOrder());
  ...
  public Memory slice(long index, long length) {
    // check buffer overflow
    checkIndex(index, length);
    return new SlicedJDK2Memory(this, index, length);
  }

  // @Override
  public Memory duplicate() {
    JDK2Memory memory = new JDK2Memory(address, capacity, byteOrder(), allocator);
  ...
  static class SlicedJDK2Memory extends JDK2Memory implements Memory.Sliced {

    private final JDK2Memory prev;

    SlicedJDK2Memory(JDK2Memory prev, long index, long length) {
